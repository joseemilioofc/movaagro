...
export const FleetDriversManager = () => {
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ name: "", phone: "", license_number: "", license_expiry: "", assigned_vehicle_id: "none", status: "active" });
    setOpen(true);
  };
  ...
  const openEdit = (d: Driver) => {
    ...
    setForm({
      ...
      assigned_vehicle_id: d.assigned_vehicle_id || "none", status: d.status,
    });
    setOpen(true);
  };
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>Gerencie os motoristas da empresa</CardDescription>
        </div>
                ...
                <Card key={d.id}>
                  <CardContent className="p-4 space-y-2">
                    ...
                    <div className="text-xs text-muted-foreground space-y-0.5">
                      {d.license_number && <p>Carta: {d.license_number}{d.license_expiry ? ` • Val. ${new Date(d.license_expiry).toLocaleDateString("pt-PT")}` : ""}</p>}
